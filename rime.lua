...
function date_translator(input, seg, env)
	...
	local str_date = {}
	str_date.year = os.date("%Y")
	str_date.month = os.date("%m")
	str_date.date = os.date("%d")

	-- Get rid of zeros
	str_date.month = str_date.month:match("0*(%d+)")
	str_date.date = str_date.date:match("0*(%d+)")
	...
	if input == date then
		...
		local cand = Candidate(
			...
			seg.start,
			seg._end,
			str_date.year .. "年" .. str_date.month .. "月" .. str_date.date .. "日",
			""
		)
		cand.quality = 100
		yield(cand)
